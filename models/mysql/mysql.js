...
export class MovieModel {
  static async getAll ({ genre }) {
    let query =
      'SELECT title, year, director, duration, poster, rate, BIN_TO_UUID(movies.id) id FROM movies'

    if (genre) {
      const lowerCaseGenre = genre.toLowerCase()
      query +=
        ' INNER JOIN movie_genres ON movies.id = movie_genres.movie_id INNER JOIN genres ON movie_genres.genre_id = genres.id WHERE LOWER(genres.name) = ?;'
  ...
  static async create ({ input }) {
    ...
    const genreQuery =
      'INSERT INTO movie_genres (movie_id, genre_id) VALUES ((SELECT id FROM movies WHERE title = ?), (SELECT id FROM genres WHERE name = ?));'
